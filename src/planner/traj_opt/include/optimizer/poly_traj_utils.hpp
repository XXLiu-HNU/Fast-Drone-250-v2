...
namespace poly_traj
{
    ...
    {
    private:
        double duration;                                        //轨迹段的持续时间，以秒为单位。
        CoefficientMat coeffMat;                                //系数矩阵，表示位置轨迹的多项式系数

    public:
        Piece() = default;

        Piece(double dur, const CoefficientMat &cMat)           //用指定的持续时间和系数矩阵初始化对象
            : duration(dur), coeffMat(cMat) {}

        inline int getDim() const                               //返回轨迹的维度（默认为3）。
        {
            return 3;
        }

        inline int getOrder() const                             //getOrder(): 返回轨迹的多项式阶数（默认为5）。
        {
            return 5;
        }
        ...
        inline CoefficientMat normalizePosCoeffMat() const  //获取归一化的位置系数矩阵。
        {
        ...
        inline VelCoefficientMat normalizeVelCoeffMat() const  //获取归一化的速度系数矩阵。
        {
        ...
        inline AccCoefficientMat normalizeAccCoeffMat() const   //获取归一化的加速度系数矩阵。
        {
        ...
        inline double getMaxVelRate() const  // 返回轨迹的最大速度。
        {
        ...
        inline double getMaxAccRate() const //返回轨迹的最大加速度
        {
        ...
        inline bool checkMaxVelRate(const double &maxVelRate) const //检查轨迹是否满足给定的最大速度约束。
        {
        ...
        inline bool checkMaxAccRate(const double &maxAccRate) const //检查轨迹是否满足给定的最大加速度约束。
        {
        ...
        // GaaiLam
        //计算给定点 pt 在轨迹上的最近点（投影点），返回投影点的时间 tt 和位置 pro_pt。
        ...
        //计算轨迹与平面（通过点 p 且法向量为 v）的交点，返回交点的时间 tt 和位置 pt。
    ...
    public:
        ...
        // The input b is required to be N*m, i.e.,
        // m vectors to be solved.
        //solveAdj：求解转置系统 𝐴𝑇𝑥=𝑏，算法类似于 solve
