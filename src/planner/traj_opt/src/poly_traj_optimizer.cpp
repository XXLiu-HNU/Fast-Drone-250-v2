...
namespace ego_planner
{
  /* main planning API */
  // 是一个实现多段轨迹优化的函数，
  // 采用了L-BFGS优化算法来解决轨迹平滑性、避障性和群体间隔等多目标的非线性优化问题
      ...
      double &final_cost)
  {
    ...
    wei_swarm_mod_ = wei_swarm_;  //表示多无人机间的避碰权重
    ...
    min_ellip_dist2_.resize(swarm_trajs_->size()); //存储与其他无人机的最小距离
      ...
      int id_cps_end, PtsChk_t &pts_check)
  {
    ...
    while (true)
    {
      ...
      {//如果 t 很小（接近起点）或者当前检查点的列表为空
       //或者当前位置与上一个位置的差距大于一半分辨率（RES_2），则将当前位置添加到 pts_check 中
    ...
    return true;
    //函数的主要任务是从给定的轨迹中采样出需要检查的点，并将其存储在 pts_check 中
    //每个点都是在轨迹上按照一定的时间间隔（由轨迹段的持续时间和分辨率限制）采样出来的
    //该过程还考虑了点之间的最小距离，确保轨迹点的间隔符合要求。
  }
  ...
  {
    // set some param from NodeHandle
